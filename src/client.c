...
void interactive_mode(int sock) {
    ...
    while (1) {
        ...
        if (FD_ISSET(STDIN_FILENO, &readfds)) {
            ...
            // Si el usuario escribió QUIT, podemos cortar localmente o esperar que el server cierre
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    ...
    fprintf(stderr, "One-Shot mode:    ./client 127.0.0.1 8080 admin:1234 METRICS\n");
    return 1;
  }
  ...
  if (argc > 4) {
      ...
      while ((n = recv(sock, buf, BUF_SIZE - 1, 0)) > 0) {
        buf[n] = 0;
        printf("%s", buf);
        // Si el buffer parece terminar el comando, cortamos para no quedarnos colgados
        // si el server no cierra la conexión.
        // Pero lo ideal es usar el modo interactivo.
        break; 
      }
  } else {
      // --- MODO INTERACTIVO ---
